-- Fix Existing Events and Create Packages
-- This script will update existing events and create packages for them

-- First, let's see what events currently exist
...
DECLARE
    ...
    event_id INTEGER;
BEGIN
    -- Loop through all existing events and create packages for each
    FOR event_record IN SELECT id, name, service_categories FROM events LOOP
...
SELECT 'Packages Created:' as info;
SELECT 
...
SELECT 'Package Count per Event:' as info;
SELECT 
  e.name as event_name,
