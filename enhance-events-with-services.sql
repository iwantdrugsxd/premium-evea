...
-- Step 9: Count total services across all events
SELECT 'Service Summary:' as info;
SELECT 
  COUNT(DISTINCT name) as total_events,
  COUNT(DISTINCT unnest(service_categories)) as unique_services,
  array_length(ARRAY(
    SELECT DISTINCT unnest(service_categories) 
    FROM events
  ), 1) as total_service_types
FROM events;
